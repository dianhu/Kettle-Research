...
public class ReplaceString extends BaseStep implements StepInterface {
    ...
    throws KettleException {
        ...
        /*
         * 处理第一条数据时做的事情：
         * 1.取得inputRowMeta,读取Step中的配置信息，把Out Stream field加入到outRowMeta.
         * 2.读取Step的配置信息，即step meta信息，把patterns,replaceByString信息存储到数组里面.
         *   所有的数据都放到ReplaceStringData中.
         */
        ...
        if (first) {
            ...
            //把新增的valueMeta加入到outputRowMeta-->Hcy
        ...
        try 
        {
            ...
            //把新的outputRowMeta和新的Object[]放入到和下一个Step之间的RowSet中
