...
public class RowGenerator extends BaseStep implements StepInterface
{
	...
	/**
	 * 构建RowMetaAndDate数据结构，RowMeta里有vlaueList,即每个列的列名和列的数据类型，类似数据库建表时的结构，(Name String,Score int)
	 * RowData是一个Object[]数组，里面存放了一行的所有数据，每列对应的数据是一个object对象。
     *
	 */
   ...
   /**
    * During initialization PDI calls the init() method of the step once. After all steps have initialized, PDI calls
    * processRow() repeatedly until the step signals that it is done processing all rows. After the step is finished processing
    * rows, PDI calls dispose().
    */
